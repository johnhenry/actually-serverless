...
self.addEventListener("message", (e) => {
  ...
  switch (e.data.type) {
    case "host-start":
      ...
      break;
    case "host-start-func":
      e.waitUntil(StartHost(e, true));
      break;
    case "host-stop":
...
async function StartHost(e, func = false) {
...
self.addEventListener("fetch", async (e) => {
  ...
  if (!scopeURLMatch) return; // not part of a host URL
  // Strip host name from URL and get the URL within the host
  const hostName = scopeURLMatch[1];
  ...
  e.respondWith(HostFetch(hostName, hostRelativeUrl, e.request));
});

async function HostFetch(hostName, url, request) {
  const { method, headers, body = null } = request;
  // "body" not available. see: https://bugs.chromium.org/p/chromium/issues/detail?id=688906
...
function HostNotFoundResponse(hostName) {
  return new Response(
    `<h1>Host not found</h1><p>The host '<em>${hostName}</em>' does not appear to be running. Make sure you have chosen a folder to serve. Alternatively you might have closed the host's browser tab.</p>`,
    {
      status: 404,
...
function ClientNotFoundResponse(hostName) {
  return new Response(
    `<h1>Client not found</h1><p>A client for the host '<em>${hostName}</em>' does not appear to be running. You might have closed its browser tab.</p>`,
    {
      status: 404,
      statusText: "Not Found",
      headers: { "Content-Type": "text/html" },
...
function FetchFailedResponse(hostName, url) {
  return new Response(
    `<h1>File not found</h1><p>The host '<em>${hostName}</em>' was not able to return a file for the path '<em>${url}</em>'. Check the file exists in the folder you chose to serve.`,
    {
      status: 404,
      statusText: "Not Found",
      headers: { "Content-Type": "text/html" },
